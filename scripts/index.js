...
(function () {
  var panel = {
    ...
    createHueCanvas: function createHueCanvas() {
      this.elm.hue.x = 0;
      this.elm.hue.y = 0;
    ...
    createColorCanvas: function createColorCanvas() {
    ...
    createAlphaCanvas: function createAlphaCanvas() {
    ...
    drawHueCanvas: function drawHueCanvas(parent, elm) {
    ...
    drawColorCanvas: function drawColorCanvas(parent, elm) {
      var ctx = elm.getContext("2d");
      var imgdata = ctx.getImageData(0, 0, parent.width, parent.height);
      var imgdatalength = imgdata.data.length;
      for (var i = 0; i < imgdatalength / 4; i++) {
    ...
    drawAlphaCanvas: function drawAlphaCanvas(parent, elm) {
      var ctx = elm.getContext("2d");
      var imgdata = ctx.getImageData(0, 0, parent.width, parent.height);
      var imgdatalength = imgdata.data.length;
      for (var i = 0; i < imgdatalength / 4; i++) {
    ...
    hueGradient: function hueGradient(parent, x, y, hue) {
      var colorValue;
    ...
    colorGradient: function colorGradient(color, parent, x, y) {
      return Math.floor((255 - (255 - color) * (x / parent.width)) * ((parent.height - y) / parent.height));
    },
    createElm: function createElm(elem, parent, className) {
    ...
    getElm: function getElm(elm, type) {
      var a = this.elm[type];
      a.elm = elm;
      ...
      if (a.x < 0) {
        a.x = a.elm.scrollLeft;
      };
      if (a.x > a.rect.width) {
        a.x = a.elm.scrollLeft + a.rect.width;
      };
      if (a.y > a.rect.height) {
        a.y = a.elm.scrollTop + a.rect.height;
      };
      if (a.y < 0) {
        a.y = a.elm.scrollTop;
      };
    ...
    updateRGBA: function updateRGBA(parent) {
    ...
    updateHue: function updateHue(parent) {
    ...
    updatePicker: function updatePicker(type) {
      if (type === "hue") {
    ...
    updatePickerCoords: function updatePickerCoords() {
    ...
    textColorCheck: function textColorCheck() {
    ...
    getHueAndCoords: function getHueAndCoords() {
    ...
    inputCheck: function inputCheck(elm, n, color) {
      if (elm.dataset.color === "r" || elm.dataset.color === "g" || elm.dataset.color === "b" || elm.dataset.color === "a") {
        n = Number(n);
        if (isNaN(n) || n < 0 || n > 255) {
          alert("Insert a valid number value!");
          return this[color];
        } else if (color === "a" && n > 1) return 1;else if (color === "a") return n.toFixed(2);else return Math.floor(n);
      } else if (elm.dataset.color === "hex") {
        if (n.indexOf("#") !== 0) n = n.replace(/^/, '#');
        if (/(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(n)) return n;else return this.hex;
      }
    },
    ...
    mixWhiteRgba: function mixWhiteRgba(r, g, b, a) {
  ...
  panel.elements.input.forEach(function (input) {
